...
for n in range(6):
    duration = float(5000)
    tdump=open('avg_balance2.csv','w')
    print(n)
    range1 = (130, 200)
    supply_schedule = [{'from': start_time, 'to': duration, 'ranges': [range1], 'stepmode': 'jittered'}
                    ]

    range2 = (75, 200)
    demand_schedule = [{'from': start_time, 'to': duration, 'ranges': [range2], 'stepmode': 'jittered'}
                    ]
